...
def seed_data():
    ...
    try:
        from app.database.models import (
            Tenant, User, Group, Role, Service, Module, Policy,
            user_tenant, group_user, group_role, user_role,
            Department, Employee, Cutoff, Shift, Vendor
        )
        ...
        try:
            ...
            session.add(tenant)
            session.flush()  # ensures tenant.tenant_id is available
            ...
            modules = [
                ...
                Module(service_id=services[0].id, name="vehicle_management", description="Manage vehicles"),
                Module(service_id=services[0].id, name="vehicle_type_management", description="Manage vehicle types"),
                Module(service_id=services[0].id, name="vendor_management", description="Manage vendors"),
                Module(service_id=services[0].id, name="admin_dashboard", description="View system dashboards"),
                Module(service_id=services[0].id, name="company_dashboard", description="View company dashboards"),
            ...
            users = [
                User(username="admin", email="admin@gmail.com",
                     hashed_password="a9dc602f9d82bc6720b2b4bb016edcacf7da4b2b453a466b742da743f3cba15d",
                     tenant_id=tenant.tenant_id, is_active=1, mobile_number="9876543210"),
                User(username="manager", email="manager@gmail.com",
                     hashed_password="a9dc602f9d82bc6720b2b4bb016edcacf7da4b2b453a466b742da743f3cba15d",
                     tenant_id=tenant.tenant_id, is_active=1, mobile_number="9876543211"),
                User(username="driver", email="driver@gmail.com",
                     hashed_password="a9dc602f9d82bc6720b2b4bb016edcacf7da4b2b453a466b742da743f3cba15d",
                     tenant_id=tenant.tenant_id, is_active=1, mobile_number="9876543212")
            ]
            session.add_all(users)
            session.flush()
            ...
            SEED_employees = [
                Employee(
                    ...
                    mobile_number="9123456789",
                    hashed_password="a9dc602f9d82bc6720b2b4bb016edcacf7da4b2b453a466b742da743f3cba15d",
                    employee_code="acm1", tenant_id=tenant.tenant_id,
                    department_id=departments[0].department_id, gender="Female",
                    alternate_mobile_number="9123456789", office="Head Office",
                    subscribe_via_email=True, subscribe_via_sms=False,
                    address="Acme HQ, Tech Park", longitude="77.4889504", latitude="13.0690214",
                    landmark="Near Big Mall", is_active=True
                ),
                Employee(
                    name="Bob Smith", email="bob.smith@example.com",
                    mobile_number="9123456781",
                    hashed_password="a9dc602f9d82bc6720b2b4bb016edcacf7da4b2b453a466b742da743f3cba15d",
                    employee_code="sta1", tenant_id=tenant.tenant_id,
                    department_id=departments[1].department_id, gender="Male",
                    office="Main Office", special_need="Wheelchair Access",
                    subscribe_via_email=True, subscribe_via_sms=True,
                    address="Startup Inc, Downtown", longitude="77.5945665", latitude="13.0674392",
                    landmark="Opposite Metro Station", is_active=True
                ),
            ]
            session.add_all(SEED_employees)
            session.flush()
            ...
            # Groups
            groups = [
                Group(group_name="Administrators", tenant_id=tenant.tenant_id, description="System administrators"),
                Group(group_name="Company Managers", tenant_id=tenant.tenant_id, description="Company managers"),
                Group(group_name="Drivers", tenant_id=tenant.tenant_id, description="Vehicle operators")
            ]
            session.add_all(groups)
            session.flush()
            ...
            # Roles
            roles = [
                Role(role_name="Super Admin", description="Full system access", tenant_id=tenant.tenant_id),
                Role(role_name="Fleet Manager", description="Fleet management access", tenant_id=tenant.tenant_id),
                Role(role_name="Driver", description="Vehicle operator access", tenant_id=tenant.tenant_id)
            ]
            session.add_all(roles)
            session.flush()

            # Policies (trimmed example, full list continues like your original)
            policies = [
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[0].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[1].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[2].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[3].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[4].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[5].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[6].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[7].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[8].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[9].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[10].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[11].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[12].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[13].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[14].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[15].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[16].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[17].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[18].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[0].id, module_id=modules[19].id,
                      can_view=True, can_create=True, can_edit=True, can_delete=True,
                      group_id=groups[0].group_id, condition={"ip_range": "10.0.0.0/8"}),

            
                Policy(tenant_id=tenant.tenant_id, service_id=services[1].id, module_id=modules[1].id,
                    can_view=True, can_create=True, can_edit=False, can_delete=False,
                    group_id=groups[1].group_id, condition={"ip_range": "10.0.0.0/8"}),
                Policy(tenant_id=tenant.tenant_id, service_id=services[1].id, module_id=modules[2].id,
                    can_view=True, can_create=True, can_edit=False, can_delete=False,
                    group_id=groups[1].group_id, condition={"ip_range": "10.0.0.0/8"}),

                Policy(tenant_id=tenant.tenant_id, service_id=services[1].id, module_id=modules[13].id,
                    can_view=True, can_create=True, can_edit=False, can_delete=False,
                    group_id=groups[1].group_id, condition={"ip_range": "10.0.0.0/8"}),

                Policy(tenant_id=tenant.tenant_id, service_id=services[1].id, module_id=modules[15].id,
                    can_view=True, can_create=True, can_edit=False, can_delete=False,
                    group_id=groups[1].group_id, condition={"ip_range": "10.0.0.0/8"}),
            ...
            for user in users:
                session.execute(user_tenant.insert().values(
                    user_id=user.user_id, tenant_id=tenant.tenant_id, metadata={"role": "primary"}
                ))
            ...
            # Group-Role mappings
            session.execute(group_role.insert().values(group_id=groups[0].group_id, role_id=roles[0].role_id))
            session.execute(group_role.insert().values(group_id=groups[1].group_id, role_id=roles[1].role_id))
            session.execute(group_role.insert().values(group_id=groups[2].group_id, role_id=roles[2].role_id))

            # User-Role mappings
            session.execute(user_role.insert().values(user_id=users[0].user_id, role_id=roles[0].role_id, tenant_id=tenant.tenant_id))
            session.execute(user_role.insert().values(user_id=users[1].user_id, role_id=roles[1].role_id, tenant_id=tenant.tenant_id))
            session.execute(user_role.insert().values(user_id=users[2].user_id, role_id=roles[2].role_id, tenant_id=tenant.tenant_id))
